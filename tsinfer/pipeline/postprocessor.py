...
class Postprocessor(StoppableIteratingBuffer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.initialize(
            batch_size, kernel_size, kernel_stride, fs, **postprocessing_fn_kwargs
        )
    ...
    def initialize(self, batch_size, kernel_size, kernel_stride, fs, **kwargs):
        ...
        self._prediction = np.zeros((num_samples_total,), dtype=np.float32)
        ...
        self._aggregation_weights = (
            np.ones((batch_size, num_samples_frame), dtype=np.float32)
            / batch_size
        )
        for i in range(batch_size):
            idx = list(range(batch_size - 1 - i)) + list(range(-i, 0))
            weights = list(range(i + 1, batch_size)) + list(
                range(batch_size - 1, batch_size - i - 1, -1)
            )
